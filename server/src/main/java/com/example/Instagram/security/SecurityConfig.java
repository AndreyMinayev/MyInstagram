...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        //   for exceptions setting  jwtAuthenticationEntryPoint class  responsible
        http.cors().and().csrf().disable()
                .exceptionHandling().authenticationEntryPoint(jwtAuthenticationEntryPoint)
                .and().sessionManagement()
                .sessionCreationPolicy(SessionCreationPolicy.STATELESS)
                // allow any requests to  Sign_Up_Urls
                .and().authorizeRequests()
                .antMatchers(SecurityConstants.SIGN_UP_URLS).permitAll()
                // but any other need to be authenticated
    ...
    @Override
    @Bean(BeanIds.AUTHENTICATION_MANAGER)  // overriding just for that annotation
